...
class AudioProcessor:
    def __init__(self):
        self.transform = nn.Sequential(
            ...
            T.AmplitudeToDB() # Convert to decibels
        )
    def process_audio_chunk(self, audio_data):
        waveform = torch.from_numpy(audio_data).float() # Convert to tensor

        waveform = waveform.unsqueeze(0) # Add batch dimension because the model expects input shape [batch_size, channels, time]

        spectrogram = self.transform(waveform) # Apply the transformation

        return spectrogram.unsqueeze(0) # Add channel dimension because the model expects input shape [batch_size, channels, height, width]
...
class AudioClassifier:
    ...
    def inference(self, request: InferenceRequest):
        # For production, frontend -> upload file to S3, inference endpoint -> download file from S3 bucket
        # For testing, frontend -> send file directly, inference endpoint -> process file directly
